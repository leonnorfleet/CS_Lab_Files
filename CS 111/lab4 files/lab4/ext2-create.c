...
void write_superblock(int fd) {
	...
	superblock.s_blocks_per_group = NUM_BLOCKS; /* One group*/
	superblock.s_frags_per_group = NUM_BLOCKS; /* One group*/
	superblock.s_inodes_per_group = NUM_INODES; /* One group*/
...
void write_block_bitmap(int fd)
{
	...
	int i;
	for (i = 0; i < BLOCK_SIZE; ++i) { /* bit masking to set up bitmap block states */
		/* only blocks 24 - 1023 are free */
		if (i < 2 || i > 127) {
...
void write_inode_bitmap(int fd)
{
	...
	// TODO It's all yours
	u8 map_value[BLOCK_SIZE];
	for (int i = 0; i < BLOCK_SIZE; ++i) { /* bit masking to set up bitmap inode states */
		/* only inodes 14 - 128 are free */
		if (i < 1 || i > 15) {
			map_value[i] = 0b11111111;
		}
		else {
			map_value[i] = (i == 1) ? 0b00011111 : 0b00000000;
		}
	}
...
void write_inode_table(int fd) {
	...
	hello_world_inode.i_blocks = 2; /* 512 bytes */
...
void write_hello_world_file_block(int fd)
{
	...
	uint32_t size = write(fd, "Hello world\n", 12);
